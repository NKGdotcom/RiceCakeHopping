...
public class RiceCakeManager : MonoBehaviour
{
    ...
    private ParticleSystem riceCakeParticle;
    ...
    public void OnRiceCakeCollision(GameObject _oneRiceCake,GameObject _oneMoreRiceCake)
    {
        if(stageRiceCakeList.Contains(_oneRiceCake) && stageRiceCakeList.Contains(_oneMoreRiceCake))
        {
            ...
            {
                if(riceCakeParticle == null) riceCakeParticle = (Instantiate(collisionParticle, _particleInstantiatePosition, Quaternion.identity)).GetComponent<ParticleSystem>();
                else
                {
                    riceCakeParticle.transform.position = _particleInstantiatePosition;
                    riceCakeParticle.Play();
                }
            }
