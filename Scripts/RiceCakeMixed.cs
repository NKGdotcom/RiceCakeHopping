...
public class RiceCakeMixed : MonoBehaviour
{
    ...
    private void ChangeTaste(string _tag, Material _material, GameObject _particle, string _newTag, ref bool _tasteFlag)
    {
        SoundManager.Instance.PlaySE(SESource.riceCakeUnionAndButton);
        _tasteFlag = true;

        if (_tag == "Kinako" && !soySourceTaste || _tag == "SoySource" && !kinakoTaste)
        {
            ...
            if (riceCakeRenderer != null && _material != null)
            {
                riceCakeRenderer.material = _material;
            }
    ...
    {
        if (collision.gameObject.CompareTag("Normal"))//餅同士はくっつく
        {
